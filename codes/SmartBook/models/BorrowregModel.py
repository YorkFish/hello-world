...
class Borrowreg(SuperModel):
    ...
    @property
    def properties(self):
        return [str(self.id), str(self.book_id), str(self.borrow_user_id),
                self.state, str(self.return_user_id)]

    def to_file_str(self):
        return self.FILE_SEPARATOR.join(self.properties) + '\n'

    def __str__(self):
        s = ', '.join(self.properties)
        return f"Borrowreg:({s})"
...
class BorrowregDAO(SuperModel):
    ...
    用来访问和处理借阅记录数据
    """
    FILE_NAME = os.path.join(os.path.abspath(os.path.dirname(os.path.abspath(__file__))),
                             "Borrowreg.dbx")
    ...
    def insert(p_borrow):
        ...
        with open(BorrowregDAO.FILE_NAME, 'a',
    ...
    def select(p_state=None, p_borrow_user_id=None):
        ...
        if os.path.exists(BorrowregDAO.FILE_NAME):
            with open(BorrowregDAO.FILE_NAME, 'r',
        ...
        if p_borrow is not None and p_borrow.id > 0:
            ...
            for borrow in borrows:
                if borrow.id == p_borrow.id:
                    ...
                    logger.info(borrow)
                    break
            with open(BorrowregDAO.FILE_NAME, 'w',
